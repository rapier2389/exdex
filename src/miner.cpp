...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("EXDMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("EXDMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("EXDMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("exdex-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        if (fProofOfStake) {
            ...
            if (!pblock->SignBlock(*pwallet)) {
                LogPrintf("EXDMiner(): Signing new block failed \n");
                continue;
            }
        ...
        LogPrintf("Running EXDMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("ThreadEXDMiner() exception");
    } catch (...) {
        LogPrintf("ThreadEXDMiner() exception");
    }

    LogPrintf("ThreadEXDMiner exiting\n");
}
